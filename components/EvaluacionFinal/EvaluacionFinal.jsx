  ...
  async function stopRecordingExpo() {
    ...
    try {
      ...
      if (idLeccion == lecciones.length - 1) {
        ...
        router.push({
          pathname: "/evaluacionFinalCompletada",
          params: {
  ...
  async function fetchPhrasesFromApi() {
    try {
      ...
      if (data && Array.isArray(data)) {
        // Filtra las lecciones por tipoLeccion 'EF'
        const leccionesEF = data.filter(
          (leccion) => leccion.tipoLeccion === "EF"
        );

        // Verificamos que haya al menos una lección de tipo "EF"
        if (leccionesEF.length > 0) {
          const evaluaciones = leccionesEF[0].contenido?.Evaluacion;
          console.log("Evaluacion final ", leccionesEF[0]?.id);
          setIdEvaluacion(leccionesEF[0]?.id);
